...
namespace softeng1
{
    public partial class warrantyForm : Form
    {
        ...
        {
            String query = "SELECT firstname, lastname ,PRODUCT_NAME, ORDER_DATE, order_WARRANTY, warranty_status FROM PERSON, CUSTOMER, SALES_ORDER, PRODUCT WHERE ORDER_CUSTOMER_ID = CUSTOMER_ID AND CUSTOMER_PERSON_ID = PERSON_ID AND ORDER_PRODUCT_ID = PRODUCT_ID AND ORDER_STATUS = 'Paid' and person_type = 'customer'";
            conn.Open();
            ...
            warrantyData.Columns["ORDER_STATUS"].HeaderText = "Status";
        }
        ...
        {
            String query = "SELECT FIRSTNAME, LASTNAME, PRODUCT_NAME, ORDER_DATE, ORDER_WARRANTY, WARRANTY_STATUS FROM person, product, sales_order WHERE (CONCAT(FIRSTNAME,' ', LASTNAME) LIKE '%" + custnameTxt.Text + "%') AND ORDER_PRODUCT_ID = PRODUCT_ID AND ORDER_STATUS = 'Paid' AND PERSON_ID = CUSTOMER_PERSON_ID AND PERSON_TYPE = 'CUSTOMER'";
